...
const PatientDashboard = () => {
  ...
  const fetchUserResponses = async (email) => {
    try {
      const q = query(collection(db, "patients", email, "responses"), where("userEmail", "==", email));
      const querySnapshot = await getDocs(q);
    ...
    <div className="flex-grow p-6">
      <div className="rounded-lg p-6 bg-transparent">
        ...
        <div className="bg-white shadow-lg rounded-lg p-4 border border-purple-100 mb-8 overflow-x-auto">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-700">{patientData?.location || "N/A"}</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="font-medium text-purple-700">Gender:</span>
              <span className="text-gray-700">{patientData?.gender || "N/A"}</span>
            </div>
            <div className="flex items-center space-x-2">
        ...
        <div className="bg-white shadow-lg rounded-lg p-4 border border-purple-100 mb-8 overflow-x-auto">
          ...
          {userResponses.length > 0 ? (
            userResponses.map((response) => (
              <div key={response.id} className="border-b border-gray-200 py-2">
        ...
        <div className="mb-8">
          ...
          {loading ? (
            <div className="py-8 text-center text-gray-500">Loading your stories...</div>
          ) : error ? (
                ...
                <ListViewCard
                  ...
                  date={
                    story.date_time && story.date_time.seconds
                      ? new Date(story.date_time.seconds * 1000).toLocaleDateString()
                      : "No date available"
                  }
