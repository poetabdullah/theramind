...
const Questionnaire = () => {
  ...
  const questions = [
    {
      ...
      options: [
        ...
        { label: "Obsessing over some particular things & feeling the need to make things “just right”", condition: "OCD" },
      ],
    },
    {
      text: "Are you facing any of the following symptoms these days?",
      options: [
        { label: "Feeling overburdened by tasks & responsibilities", condition: "Stress" },
        { label: "Feeling anxious & experiencing palpitations (racing of the heartbeat)", condition: "Anxiety" },
        { label: "Feeling tired for most of the day even when you haven’t done anything", condition: "Depression" },
        { label: "Having nightmares & feeling startled/frightened about a traumatic incident", condition: "Trauma" },
        { label: "Feeling irritated when things are not going a certain way", condition: "OCD" },
      ]
    },
    {
      ...
      options: [
        ...
        { label: "Frequently feeling sad, lonely & empty", condition: "Depression" },
        { label: "Feeling fearful thinking about a past event", condition: "Trauma" },
        { label: "Feeling the urge to make things symmetrical & clean", condition: "OCD" },
      ]
    },
    {
      text: "Which of the following behavioral symptoms have you been facing lately?",
      options: [
        { label: "Changes in eating habits (overeating & loss of appetite)", condition: "Stress" },
        { label: "Avoiding situations that trigger anxiety", condition: "Anxiety" },
        { label: "Withdrawal from social gatherings", condition: "Depression" },
        { label: "Risk-taking or self-destructive behavior", condition: "Trauma" },
        { label: "Excess cleaning, constantly arranging & repeatedly checking things", condition: "OCD" },
      ]
    },
  ]
  ...
  const handleNext = () => {
    ...
    if (currentQuestion === 1 && responses.none && responses.symptoms === "No") {
      setCurrentQuestion(3);
      return;
    }

    // Skip Question 3 if "None" is not selected and symptoms = "Yes"
    if (currentQuestion === 2 && !responses.none && responses.symptoms === "Yes") {
  ...
  const isBackButtonDisabled = () => {
    if (currentQuestion === 3 && responses.none && responses.symptoms === "No") {
      return true;
    }
    if (currentQuestion === 5 && responses.suicidalThoughts === "Yes") {
      return true;
      ...
      <main className="flex-grow flex items-center justify-center mb-5">
        ...
        >
          ...
          {currentQuestion === 1 && (
            <div>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                { name: "none", label: "None of the above" }].map((option) => (
                  <label key={option.name} className="flex items-center space-x-2 bg-blue-100 p-2 rounded-full cursor-pointer hover:bg-green-200">
                    <input
                      type="checkbox"
                      name={option.name}
          ...
          <div className="flex justify-between mt-6">
            {currentQuestion > 1 && (
              <button className={`bg-gray-300 p-2 rounded hover:bg-gray-400 ${isBackButtonDisabled() ?
                "opacity-50 cursor-not-allowed" : ""}`} onClick={handlePrevious}>Back</button>
            )}
            <button
              className={`bg-blue-500 p-2 rounded text-white hover:bg-blue-600 ${isNextButtonDisabled() ?
