...
const Questionnaire = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      responses["question_question3"] === "yes_suicidalthoughts"
    ) {
  ...
  const handleNext = () => {
    if (!detectedCondition && currentQuestionIndex === 6) {
      ...
      const detected = Object.keys(conditionCounts).reduce((a, b) =>
      ...
      if (selectedOption && currentQuestion.category) {
        ...
        setSubtypeScores((prev) => ({
          ...
          [currentQuestion.category]:
            (prev[currentQuestion.category] || 0) + score,
        }));
      }
  ...
  const handlePrevious = () => {
    ...
    if (
      detectedCondition &&
      currentQuestionIndex >= conditionRanges[detectedCondition].start
    ) {
    ...
    return (
      <div className="flex flex-col min-h-screen bg-gray-50">
        ...
        <main className="flex-grow flex items-center justify-center mb-5">
          <motion.div
            className="bg-white shadow-lg rounded-lg p-6 w-full max-w-xl text-center"
            {...scaleEffect}
          >
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-grow flex items-center justify-center mb-5">
        <motion.div
          className="bg-white shadow-lg rounded-lg p-6 w-full max-w-xl relative"
          {...scaleEffect}
        >
          <AnimatePresence>
            {noConditionDiagnosed && (
              <motion.div {...fadeInUp} className="text-center">
                ...
                <p className="text-lg text-orange-500">
                  You are not diagnosed with any mental health condition within
                  TheraMind's scope.
                </p>
              </motion.div>
            )}
            {suicidalThoughts && (
              <motion.div {...fadeInUp} className="text-center text-red-800">
                ...
                <p>
                  Your responses indicate suicidal thoughts. Please seek
                  immediate help from an emergency hotline.
                </p>
                ...
                <p className="mt-4">
                  If in immediate danger, dial <strong>911</strong>.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            {!noConditionDiagnosed && !suicidalThoughts && (
              <motion.button
                className={`p-2 rounded transition duration-300 ${
                  responses[`question_${questions[currentQuestionIndex]?.id}`]
